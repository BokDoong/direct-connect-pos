...
| `PosOrderSynchronizer` / `PosStockFinder` / `PosStoreRegistrar` / `PosOrderWriter` | 앱 | **파트너 타입 분기 지점** (D10·D11) — 아웃바운드 3종 + 매핑 저장. INTEGRATED는 DirectPosPartner 전략, FOODTECH·HAPPYORDER는 레거시 클라이언트, KARROT은 no-op. exhaustive when으로 새 타입 추가가 컴파일 타임에 잡힘 | 레거시(파트너 주도 규격) 경로와의 호환 계층 — 직연동 재설계를 레거시에 강요하지 않음 |
