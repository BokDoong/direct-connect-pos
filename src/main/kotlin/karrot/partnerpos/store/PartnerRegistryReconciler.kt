 ...
 *
 * 파트너 키는 DB(partners.name)와 코드(PartnerKey 선언) 두 곳에 정의되는 **분산 enum**이다 —
...
class PartnerRegistryReconciler(
    ...
    fun reconcile() {
        val dbNames = partnerRecords.findAllNames().toSet()
        val codeNames = registry.keys.map { it.name }.toSet()

        val notImplemented = dbNames - codeNames   // row는 있는데 구현이 없다 → 주문이 터질 파트너
        val notRegistered = codeNames - dbNames    // 구현은 있는데 row가 없다 → FK를 걸 수 없는 파트너
