...
class OrderPlacementServiceTest {
    ...
    private val registry = DirectPosPartnerRegistry(listOf(partner))

    @Test
    @DisplayName("정상 흐름: 파트너 등록 후 매핑이 저장된다")
    fun placeRegistersAndSaves() {
        val repository = InMemoryPartnerOrderMappingRepository()
        val service = OrderPlacementService(registry, repository)
        val order = sampleOrder()

        service.place(partner.key, order)
    ...
    fun ghostOrderCompensation() {
        ...
        val service = OrderPlacementService(registry, failingRepository)
        val order = sampleOrder()

        assertThrows<IllegalStateException> { service.place(partner.key, order) }
    ...
    fun compensationFailureDoesNotMaskCause() {
        ...
        assertThrows<IllegalStateException> { service.place(partner.key, sampleOrder()) }
    }
    ...
    fun registerFailurePropagates() {
        ...
        assertThrows<PosCommunicationException> { service.place(partner.key, sampleOrder()) }
